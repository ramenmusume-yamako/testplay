...
const BASE_GRAVITY = isPC ? 1.0 : 1.6; 
const BASE_JUMP = isPC ? -29.0 : -38.0; 
...
const initialBaseSpeed = isPC ? 6.0 : 11.0; 
...
// ★★★ 追加: 日付別のゲームスコアを保存する関数 ★★★
function saveGameScore(difficulty, score) {
    const today = new Date().toISOString().split('T')[0];
    let gameData = JSON.parse(localStorage.getItem('site_game_scores') || '{}');

    if (!gameData[today]) {
        gameData[today] = { easy: [], normal: [], hard: [] };
    }

    if (gameData[today][difficulty]) {
        gameData[today][difficulty].push({
            score: score,
            time: new Date().toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' })
        });
    }

    localStorage.setItem('site_game_scores', JSON.stringify(gameData));
}

// PCキーボード専用のキーイベントリスナー
...
// マウスがボタンに乗った時もキーボード選択のインデックスを同期させる
menuButtons.forEach((btn, idx) => {
...
function update(currentTime) {
    ...
    if (gameState === 'PLAYING') {
        ...
        if (startCallTimer > 0) {
            ...
            speedLoopTimer += (1 / 60) * dtRatio; 
            
            const wave = (1 - Math.cos((2 * Math.PI * speedLoopTimer) / 100)) / 2;
            
            const maxSpeedBonus = isPC ? 5.0 : 8.0; 
            currentSpeed = initialBaseSpeed + (maxSpeedBonus * wave);
            ...
            for (let i = obstacles.length - 1; i >= 0; i--) {
                ...
                if (pL < oR && pR > oL && pT < oB && pB > oT) {
                    ...
                    saveScore(score);
                    // ★★★ 追加: ゲームオーバー時にスコアを保存 ★★★
                    saveGameScore(DIFFICULTY, Math.floor(score));
                }
